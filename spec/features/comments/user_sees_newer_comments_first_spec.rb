...
describe "User can add a comment" do
  scenario "a user can add a comment on the job page" do
    ...
    fill_in "comment[author]", with: "Jeff"
    fill_in "comment[body]", with: "Would make shitty Astronaut"
    click_on "Submit"
    ...
    expect(page).to have_content "Comment by Jeff"
    expect(page).to have_content "Would make shitty Astronaut"

    expect("Jeff").to appear_before("Elon")
